            ...
            <div class="newTasksContainer">
                ...
                <div class="givenTasks">
                    ...
                    <table>
                        ...
                        <tbody>
                            <?php
                            if (isset($started_tasks) && count($started_tasks) > 0) {
                                foreach ($started_tasks as $started_task) {
                                    $s_doc_link = "../uploads/" . $started_task["task_document"];
                                    $short_s_t_description = substr($started_task["task_description"], 0, 50);
                                    echo "
                                <tr>
                                <td>{$started_task['id']}</td>
                                <td>{$started_task['task_title']}</td>
                                <td title='{$started_task['task_description']}'>$short_s_t_description ...</td>
                                <td><a href='$s_doc_link' target='_blank' class='docViewBtn'>View</a></td>
                                <td>{$started_task['created_at']}</td>
                                <td>{$started_task['dead_line']}</td>
                                <td style='text-transform:capitalize'>{$started_task['status']}</td>
                                <td><a href='./index.php?complete_task={$started_task['id']}' class='taskActionBtn' style='background-color: green;'>Complete</a></td>
                            </tr>";
                                }
                            }
                            ?>
